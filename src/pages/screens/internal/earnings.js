...
function Card(card_name, card_type, card_number, visible_card_number) {
    ...
    return (
        <article className="flex justify-between my-4 p-3 w-full rounded-xl bg-primary-0 dark:bg-primary-1">
            ...
            { IconButton("pencil", "self-center w-12 h-12 m-3") }
        </article>
    );
}

export default function Earnings(earnings, show_cards, new_card_mode) {
    const [earningsVisible, setEarningsVisible] = useState(false);
    const [cardNumberVisible, setCardNumberVisible] = useState(false);

    return (
        <section className="flex flex-col justify-between w-[40%] p-8 rounded-2xl bg-secondary-0 dark:bg-secondary-1 dark:text-accent-fg-1">
            <div className="h-[90%]">
                <section>
                    <div className="flex justify-between">
                        ...
                        <div>
                            { IconButton("pencil", "w-9 h-9") }
                            { IconButton(earningsVisible ? "eye-slash" : "eye", "w-9 h-9", () => { setEarningsVisible(!earningsVisible) }) }
                        </div>
                    </div>
                    <div className="flex flex-col my-4 p-3 w-full rounded-xl bg-primary-0 dark:bg-primary-1">
                        <label className="self-center text-3xl">
                ...
                <section className="flex h-[75%] flex-col">
                    <div className="flex justify-between">
                        ...
                        <div>
                            {IconButton(cardNumberVisible ? "eye-slash" : "eye", "w-9 h-9", () => {
                                setCardNumberVisible(!cardNumberVisible)
                            })}
                        </div>
                    </div>
                    <div className="overflow-y-auto scheme-dark">
                        {Card("BBVA", "0", "0123 0123 0123 0123", cardNumberVisible)}
                        {Card("Liverpool", "1", "0123 0123 0123 0123", cardNumberVisible)}
                        {Card("Soriana", "1", "0123 0123 0123 0123", cardNumberVisible)}
                    </div>
                </section>
            </div>
            <button className="self-center w-full p-3 font-bold rounded-lg bg-primary-0 dark:bg-primary-1 text-accent-fg-0 dark:text-accent-fg-1">
                { new_card_mode ? "Nueva tarjeta" : "Ver todas las tarjetas"}
            </button>
        </section>
    );
}
