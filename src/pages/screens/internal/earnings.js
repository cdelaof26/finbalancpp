    ...
    <article className="flex justify-between my-4 p-3 w-full rounded-xl bg-primary-0 dark:bg-primary-1">
      <div className="flex">
        <div className="flex flex-col mx-4">
          <GetSVG
            name="credit-card"
            classNameData="w-12 h-12 self-center"
          ></GetSVG>
      ...
      <div className="flex">
        <IconButton
          ...
          className={
            "self-center " +
            (visibleDeleteIcon ? "w-10 h-10 mx-1 my-4" : "w-12 h-12 m-3")
          }
        ...
        <IconButton
          ...
          icon="trash"
          className={
            "self-center " + (visibleDeleteIcon ? "w-10 h-10 m-1" : "hidden")
          }
        ></IconButton>
      </div>
    </article>
  ...
  return (
    <section className="flex h-[75%] flex-col">
      <div className="flex justify-between">
        <h1 className="self-center text-2xl font-bold">Mis tarjetas</h1>
        <div className={no_cards ? "hidden" : ""}>
          <IconButton
            icon="eye"
            toggleIcon="eye-slash"
            className="w-9 h-9"
            toggleVar={cardNumberVisible}
            action={setCardNumberVisible}
          ></IconButton>
        </div>
      </div>
        ...
        {no_cards ? (
          ...
          cardDetails.map((c, index) => (
            <Card
              ...
              editable={c.editable}
              visibleCardNumber={cardNumberVisible}
              visibleDeleteIcon={canDeleteCards}
...
export function newCard() {
  return {
    ...
    funds: "0",
    creditLimit: "0",
    cardPaymentDueDate: get_date(),
  };
}

export default function Earnings({
  showCards = true,
  canPaintEditAsPressed = false,
  func,
  canDeleteCards = true,
  cardData = [],
}) {
  if (func === undefined || func === null) return;
  if (cardData === undefined || cardData === null) return;
  ...
  const setProperty = (value, property, index) => {
    if (loadedCardIndex === -1 || cardDetails.length < index) return;
  ...
  if (showCards)
    action_button = (
      <button
        onClick={() => {
          func.setSidebarToggled(2);
          if (func.getCurrentPage() !== 4) func.loadCards();
          else createCard();
        }}
        className="self-center w-full p-3 font-bold rounded-lg bg-primary-0 dark:bg-primary-1 text-accent-fg-0 dark:text-accent-fg-1"
      >
        {func.getCurrentPage() !== 4
  ...
  let title = (
    <Title
      ...
      icons={["pencil", "eye"]}
      toggleIcon={[null, "eye-slash"]}
      paintPressed={[func.subPage === 1 && canPaintEditAsPressed, false]}
      toggleVar={[null, func.earningsVisible]}
      action={[
        () => {
          func.setSidebarToggled(2);
          func.loadEarningsNCards(func.subPage === 1 ? 0 : 1);
        },
        func.setEarningsVisible,
      ]}
    ></Title>
  );
  ...
  return (
    <div
      className={
        "flex flex-col justify-between p-8 rounded-2xl bg-secondary-0 dark:bg-secondary-1 dark:text-accent-fg-1 " +
        (showCards ? "w-[40%]" : "h-max")
      }
    >
      <div className={showCards ? "h-[90%]" : ""}>
        <div>
          ...
          <div className="flex flex-col my-4 p-3 w-full rounded-xl bg-primary-0 dark:bg-primary-1">
            <label
              className={
                "self-center text-3xl " +
                (func.earningsVisible ? "" : "font-bold")
              }
            >
        ...
        {showCards ? (
          <CardSection
            ...
            cardNumberVisible={func.cardNumberVisible}
            setCardNumberVisible={func.setCardNumberVisible}
