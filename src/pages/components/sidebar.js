...
function createButton(svg_name, svg_style, selected, action) {
  ...
  if (selected) svg_style += " text-accent-fg-1";

  return (
    <button
      onClick={action}
      className={
        "flex justify-center w-[70px] h-[70px] self-center " + bg_style
      }
    >
...
function toggleButton(amount, toggled) {
  let state = [];
  for (let i = 0; i < amount; i++) state[i] = toggled === i;
  return state;
}

export default function Sidebar(login_mode, container_func) {
  const svg_style =
    "w-full h-full p-4 self-center text-accent-fg-0 dark:text-accent-fg-1";
  let icons;
  ...
  const component = (
    <nav
      className={
        "flex flex-col justify-between w-[70px] " +
        (login_mode
          ? "bg-primary-0 dark:bg-primary-1"
          : "bg-secondary-0 dark:bg-secondary-1")
      }
    >
      <div>
  ...
  return [
    component,
    (id) => {
      setButtonsState(toggleButton(l, id));
    },
  ];
}
